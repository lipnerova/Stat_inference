...
## Basic xploratory data analyses
...
Boxplots of the data showing the differences and distributions of lengths of odontoblasts for each supplement on the left and for each dose on the right (see Appendix for code).
...
Table of means of length of odontoblasts for each dose of both supplements.
```{r tab_one}
...
```{r normality, eval=F}
shapiro.test(a$len[a$supp=="OJ" & a$dose == 0.5]) 
shapiro.test(a$len[a$supp=="OJ" & a$dose == 1]) 
shapiro.test(a$len[a$supp=="OJ" & a$dose == 2])
shapiro.test(a$len[a$supp=="VC" & a$dose == 0.5]) 
shapiro.test(a$len[a$supp=="VC" & a$dose == 1]) 
shapiro.test(a$len[a$supp=="VC" & a$dose == 2]) 
shapiro.test(a$len[a$supp=="OJ"]) 
shapiro.test(a$len[a$supp=="VC"]) 
shapiro.test(a$len) 
```